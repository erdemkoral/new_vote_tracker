...
function showImages(){
  ...
  while (barArea.hasChildNodes()) {
  barArea.removeChild(barArea.firstChild);
};
    makeTable();
    chart();
  }else{
...
//  results.push(itemSource);
  answered++;
  progressBar();
  localStorage.setItem(answered, JSON.stringify(foundProduct));
...
function progressBar() {
  ...
  var width = Math.floor((answered / questions) * 100);
  bar.style.width = width + '%';
  bar.innerHTML = answered + " / " + questions;
  return bar;
}
